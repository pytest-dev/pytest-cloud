...
def test_schedule(
    ...
    mocked_group.return_value.remote_exec.return_value.receive_each.return_value = [
        (
            ch1,
            {
                ...
                "virtual_memory": {"available": memory1 * 1024 ** 2},
            },
        ),
        (
            ch2,
            {
                "cpu_count": cpu_count2,
                "virtual_memory": {"available": memory2 * 1024 ** 2},
            },
        ),
    ]
    params = [
    ...
    assert all(
        tx.startswith(expected) for tx, expected in zip(config.option.tx, result)
    )
